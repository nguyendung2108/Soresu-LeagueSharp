...
using SharpDX;
using Environment = Soresu___ChoGath.Helpers.Environment;
...
namespace Soresu___ChoGath.Helpers
{
    public class CombatHelper
    {
        ...
        public static List<string> dotsHighDmg = new List<string>(new string[] { "karthusfallenonecastsound", "CaitlynAceintheHole", "zedulttargetmark", "timebombenemybuff", "VladimirHemoplague" });
        public static List<string> dotsMedDmg = new List<string>(new string[] { "summonerdot", "cassiopeiamiasmapoison", "cassiopeianoxiousblastpoison", "bantamtraptarget", "explosiveshotdebuff", "swainbeamdamage", "SwainTorment", "AlZaharMaleficVisions", "fizzmarinerdoombomb" });
        public static List<string> dotsSmallDmg = new List<string>(new string[] { "deadlyvenom", "toxicshotparticle", "MordekaiserChildrenOfTheGrave", "DariusNoxianTacticsONH" });
        ...
        public static bool HasDef(Obj_AI_Hero target)
        {
            foreach (var sSlot in SummonerSpellSlots)
            {
                ...
                if (spell.Name == "SummonerHeal" || spell.Name == "SummonerBarrier")
                {
                    if (spell.CooldownExpires - Game.Time < 1.5f)
                    {
                        return true;
                    }
